function setup() {
  //the canvas of the observer
  var observerCanvas = document.getElementById('observerCanvas');
  //sets the canvas for the camera
  var cameraCanvas = document.getElementById('cameraCanvas');
  //function returns the drawing context - which 
  //is an object that has all the drawing properties 
  //and functions you use to draw on the canvas.
  
  //this is returning the drawing context (which is an object)
  //that has all drawing properties and functions used to draw on 
  //canvas will also be rendering in a 2d context
  var observerContext = observerCanvas.getContext('2d');
  //returning the drawing context for the camera context
  //in 2d
 ...
 // default to drawing in the camera window
  //alternative to drawing in the camera window?
  ...
  function draw() {
    ...
    function drawStar(cx, cy, spikes, outerRadius, innerRadius, outlineClr, fillClr, TxU,scale) {
    var rot = Math.PI / 2 * 3;
    var x = cx;
    var y = cy;
    var z = 0;
    ...
    context.strokeSyle = outlineClr;
      var Tx = mat4.clone(TxU);
        mat4.scale(Tx,Tx,[scale,scale,scale]);
    context.beginPath();
    moveToTx([cx, cy - outerRadius, 0], Tx)
    for (i = 0; i < spikes; i++) {
    ...
    lineToTx([cx, cy - outerRadius, 0], Tx)
    context.closePath();
    context.lineWidth=5;
